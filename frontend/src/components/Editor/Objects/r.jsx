...
function setButtonPosition(container, fabricImage, OFFSET = 40, canvas) {
    const canvasWidth = canvas.getWidth();
    const canvasHeight = canvas.getHeight();
    const percentX = fabricImage.left / canvasWidth * 100;
    const percentY = fabricImage.top / canvasHeight * 100;

    // object ki position percent se wapas pixel me convert karo
    const pixelX = (percentX / 100) * canvasWidth;
    const pixelY = (percentY / 100) * canvasHeight;

    // object ke center point se bottom nikalna
...
const renderAllImageObjects = (
    ...
    function createRemoveBackgroundToggle(fabricImage, canvasId, callback, removeBg) {
        ...
        if (container) {
            ...
            // container.style.top = `${imageBottom + OFFSET}px`;
            // container.style.left = `${imageLeft}px`;
            setButtonPosition(container, fabricImage, OFFSET, canvas);
      ...
      `;
        ...
        if (
            ...
            fabric.Image.fromURL(
                ...
                (newImg) => {
                    ...
                    if (button) {
                        ...
                        setButtonPosition(button, newImg, 40, canvas);
                    }
                    if (aibutton) {
                        // const center = newImg.getCenterPoint();
                        // const imageBottom = center.y + (newImg.getScaledHeight() / 2);
                        ...
                        setButtonPosition(aibutton, newImg, 70, canvas);
                    }
            ...
            if (button) {
                ...
                // button.style.top = `${imageBottom + OFFSET}px`;
                // button.style.left = `${imageLeft}px`;
                setButtonPosition(button, existingObj, 40, canvas);
            }
            if (aibutton) {
                // const center = existingObj.getCenterPoint();
                // const imageBottom = center.y + (existingObj.getScaledHeight() / 2);
                ...
                // aibutton.style.top = `${imageBottom + OFFSET}px`;
                // aibutton.style.left = `${imageLeft}px`;
                setButtonPosition(aibutton, existingObj, 70, canvas);
            }
                ...
                (img) => {
                    ...
                    img.on("moving", () => {
                        ...
                        if (button) {
                            ...
                            setButtonPosition(button, img, 40, canvas);
                        }
                        if (aibutton) {
                            // const center = img.getCenterPoint();
                            // const imageBottom = center.y + (img.getScaledHeight() / 2);
                            ...
                            setButtonPosition(aibutton, img, 70, canvas);
                        }
                    });
