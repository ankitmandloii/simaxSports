...
// Function to detect sleeve type using Teachable Machine model
...
function ProductContainer() {
  ...
  useEffect(() => {
    ...
    if (Array.isArray(rawProducts) && rawProducts.length > 0 && selectedProducts.length === 0) {
      const productNode = rawProducts[1];
      let colors = [];
  ...
  // Detect sleeve type using Teachable Machine model
  useEffect(() => {
    if (frontBgImage) {
  ...
  // Navigate to front if sleeve not present
  useEffect(() => {
    if ((activeSide === "rightSleeve" || activeSide === "leftSleeve") && hidden) {
      dispatch(setActiveSide("front"));
      setTimeout(() => dispatch(setRendering()), 100);
    }
  }, [activeSide, hidden, dispatch]);
