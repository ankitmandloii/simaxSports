...
//     console.log("fetchsizees api is calling")
...
//   useEffect(() => {
//     console.log("products for api", products)
...
const QuantityToolbar = () => {
  ...
  const getSelectedProductsWithQuantities = () => {
    return products.flatMap((product) => {
      const selections = productState[product.id]?.selections || {};
      console.log("selections", selections);
      return Object.entries(selections)
        .filter(([size, qty]) => qty > 0) // only sizes with qty > 0
        .map(([size, qty]) => {
          const sizeInfo = product.sizes.find((s) => s.size === size);
          return {
            sku: sizeInfo?.sku,
            qty: qty,
            styleId: Number(product?.handle.split("-").reverse()[1])
          };
        });
    });
  };
  ...
  async function calculatePrice() {
    const selectedSkus = getSelectedProductsWithQuantities();
    ...
    try {
      const result = await fetchSizes(selectedSkus);
      console.log("result", result);

      if (!result || !Array.isArray(result) || result.length === 0) {
        toast.error("Failed to fetch available sizes. Please try again.");
        return;
      }

      const cannotFulfill = result.filter(item => !item.can_fulfill);

      if (cannotFulfill.length > 0) {
        toast.error(
          `Some sizes cannot be fulfilled. Please adjust the marked sizes.`,
          { autoClose: 6000 }
        );
        return;
      }
      ...
      console.error("calculatePrice failed:", e);
      toast.error("Something went wrong while calculating price.");
      return; // 🔑 prevent navigating on error
    }
  }
  ...
  // --------------------------------Fetch inventory availability for all products--------------------------
  const fetchSizes = async (selectedItems) => {
    console.log("fetchsizees api is calling", selectedItems);

    if (!selectedItems || selectedItems.length === 0) return;

    setLoading(true);
    try {
      const result = await getAvailableSizesAndQuantityFromSAndS(selectedItems);
      setAvailableSizesQuantity(result);
      console.log("result data from s&s", result);
      return result;
    } catch (err) {
      console.log("getting error while fetching the quantity ", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const isFulfillable = (sku) =>
    availableSizesQuantity.find((i) => i.sku === sku)?.can_fulfill ?? true;
    ...
    <div className={` toolbar-main-container ${style.toolbarMargin}`}>
      ...
      <div className={style.toolbarBox}>

        {
          loading ? <div className={style.loaderWrapper}>
            <div className={style.loader}></div>
            <p>Checking warehouse availability...</p>:
          </div> :
            <>{products.map((product) => {
              const key = getProductKey(product);
              return (
                          ...
                          {getAvailableSizesAndQuantity(product).map((item) => (
                            ...
                            onChange={(e) => {
                              ...
                              <input
                                ...
                                max={getMaxAvaibleQuantity(item)}
                                value={getValueOfItem(product.id, item)}
                                style={{ borderColor: isFulfillable(item.sku) ? "#ccc" : "red" }}
                                onChange={(e) => {
                                  let val = Number(e.target.value);
