...
const estimateDPI = (fileSizeKB, width, height) => {
...
const AddImageToolbar = () => {
    ...
    //   });
    // };
    const getDisplayDimensions = (img, canvasWidth, canvasHeight) => {
        if (!img?.originalWidth || !img?.originalHeight || !img?.scaleX || !img?.scaleY) {
            return {
                pixels: { width: 0, height: 0 },
                inches: { width: '0.00', height: '0.00' },
        ...
        const widthPixels = Math.round(img.originalWidth * img.scaleX);
        const heightPixels = Math.round(img.originalHeight * img.scaleY);

        // Use dynamic inches from img state (parseFloat to convert string to number)
        const printInchesWidth = parseFloat(img.widthInches) || 13.89; // Fallback if undefined
        const printInchesHeight = parseFloat(img.heightInches) || 13.89;

        // Calculate dynamic DPI using estimateDPI
        // Assume fileSize is stored in img.fileSize (in bytes); convert to KB
        const fileSizeKB = img?.fileSize ? img.fileSize / 1024 : 0; // Fallback to 0 if unavailable
        const dynamicDpi = estimateDPI(fileSizeKB, widthPixels, heightPixels);

        // Print Area DPI (canvas pixels / dynamic print inches)
        const printAreaDpiX = Math.round(canvasWidth / printInchesWidth);
        const printAreaDpiY = Math.round(canvasHeight / printInchesHeight);
        const printAreaDpi = Math.round((printAreaDpiX + printAreaDpiY) / 2);

        // Inches based on dynamic DPI
        const inchesWidth = (widthPixels / dynamicDpi).toFixed(2);
        const inchesHeight = (heightPixels / dynamicDpi).toFixed(2);

        return {
            pixels: { width: widthPixels, height: heightPixels },
            inches: { width: inchesWidth, height: inchesHeight },
            dpi: dynamicDpi, // Use dynamic DPI from estimateDPI
            dynamicDpi, // For display purposes
            printAreaDpi,
            scaleFactor: (canvasWidth / widthPixels + canvasHeight / heightPixels) / 2,
        };
    };
    ...
    const handleRangeInputSizeChange = (e) => {
        ...
        const dims = img?.originalWidth && img?.originalHeight
            ...
            : { pixels: { width: 0, height: 0 }, inches: { width: '0.00', height: '0.00' }, dpi: DEFAULT_TARGET_DPI };
    ...
    useEffect(() => {
        ...
        const dims = getDisplayDimensions(img, canvasWidth, canvasHeight);
                ...
                <>
                  ...
                  {/* <span><AngleActionIcon /></span> */}
                          ...
                          <p><span>Height:</span> {dims.inches.height} IN</p> */}
                                            ...
                                            <p>Dynamic DPI: {dims?.dynamicDpi} </p>
