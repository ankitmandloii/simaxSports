...
// const BASE_FILTERS = [
//     { name: 'Normal', transform: '', loading: false, image: null },
//     { name: 'Single Color', transform: '?monochrome=white', loading: false, image: null },
//     { name: 'Black/White', transform: '?sat=-100', loading: false, image: null },
// ];
...
//     function toggleFilterLoading(prevFilters, nameToToggle) {
//         setFilters(prevFilters =>
//             prevFilters.map(filter =>
//                 filter.name === nameToToggle
...
//     async function buildCanvasImage(imageSrc, color = "#fffff", selectedFilter) {
//         try {

//             console.log("buildCanvasImage function called with src", imageSrc);
//             // globalDispatch("loading", true); // Corrected the typo here

//             // Await the base64 image after processing the image
//             // setLoading(true);
//             // toggleFilterLoading(filters, selectedFilter)
//             let base64Image;
//             console.log("curent seleteced filter is ", selectedFilter, img.selectedFilter)
//             if (selectedFilter == "Single Color") {
//                 base64Image = await applyFilterAndGetUrl(imageSrc, singleColor);
...
//     useEffect(() => {
//         const preloadFilterImages = async () => {
//             setFilters(prevFilters =>
//                 prevFilters.map(f => ({ ...f, loading: true }))
//             );

//             const updatedFilters = await Promise.all(
//                 BASE_FILTERS.map(async (f) => {
//                     try {
//                         const base64 = await buildCanvasImage(
//                             buildUrl(f.transform, false, f.name),
//                             singleColor,
//                             f.name
//                         );
//                         return { ...f, image: base64, loading: false };
//                     } catch (err) {
//                         console.error("Error loading image for filter:", f.name, err);
//                         return { ...f, image: null, loading: false };
//                     }
//                 })
//             );
...
//         preloadFilterImages();
//     }, [img.src, singleColor, invertColor]);
...
//         setFilters(prev =>
//             prev.map(f =>
//                 f.name === filter.name ? { ...f, loading: true } : f
//             )
//         );

//         const base64Image = await buildCanvasImage(
//             buildUrl(filter.transform, false, filter.name),
//             singleColor,
//             filter.name
//         );
...
//             prev.map(f =>
//                 f.name === filter.name
//                     ? { ...f, loading: false, image: base64Image }
//                     : f
//             )
//         );
//     };
...
//                                             {
//                                                 f.loading
//                                                     ? <img src="https://upload.wikimedia.org/wikipedia/commons/b/b1/Loading_icon.gif" alt={f.name} className={styles.filterImage} />
//                                                     : f.image
//                                                         ? <img src={f.image} alt={f.name} className={styles.filterImage} />
//                                                         : <img src="https://thumbs.dreamstime.com/b/no-image-icon-vector-available-picture-symbol-isolated-white-background-suitable-user-interface-element-205805243.jpg" alt={f.name} className={styles.filterImage} />
//                                             }
...
const AddImageToolbar = () => {
    ...
    const [inches, setInches] = useState({ widthIn: "0.5", heightIn: "0.5" }); // Initial value based on 150px at 300 DPI
    //  const [activeFilter, setActiveFilter] = useState(filters[0]);
    ...
    useEffect(() => {
        ...
        setPreviewUrl(img.src || '');
        const initialInches = calculateInches(img?.width || 150, img?.height || 150, img?.scaleX || 1, img?.scaleY || 1);
    ...
    useEffect(() => {
        if (!img) return;
        const { width, height, scaleX, scaleY, dpi } = img; // Get the original image dimensions and scale factors
        const newInches = calculateInches(width, height, scaleX * rangeValuesSize, scaleY * rangeValuesSize, dpi);
        setInches(newInches);
    }, [rangeValuesSize, img]);
    ...
    // ---inches function
    const calculateInches = (originalWidthPx, originalHeightPx, scaleX, scaleY, posX, posY, angle, canvas, realPrintWidthIn = 12) => {
        if (!canvas || !canvas.backgroundImage) {
            // Fallback to fixed DPI (300) if no background image
            const dpi = 300;
            const scaledWidthPx = originalWidthPx * scaleX;
            const scaledHeightPx = originalHeightPx * scaleY;
            return {
                widthIn: (scaledWidthPx / dpi)?.toFixed(2),
                heightIn: (scaledHeightPx / dpi)?.toFixed(2),
                scalePercent: (scaleX * 100)?.toFixed(2),
                rotateDeg: angle?.toFixed(0),
                positionLeftPercent: ((posX / (canvas?.getWidth() || 1000)) * 100)?.toFixed(2), // Fallback canvas width
                positionTopPercent: ((posY / (canvas?.getHeight() || 1000)) * 100)?.toFixed(2), // Fallback canvas height
                dpi: dpi?.toFixed(0),
            };
        }

        // Calculate effective DPI based on mockup background
        const shirtWidthPx = canvas.backgroundImage.getScaledWidth(); // e.g., 400px for a mockup
        const effectiveDpi = shirtWidthPx / realPrintWidthIn; // e.g., 400 / 12 ≈ 33.33 DPI

        // Scaled pixel dimensions
        const scaledWidthPx = originalWidthPx * scaleX;
        const scaledHeightPx = originalHeightPx * scaleY;

        // Convert to inches using effective DPI
        const widthIn = (scaledWidthPx / effectiveDpi)?.toFixed(2);
        const heightIn = (scaledHeightPx / effectiveDpi)?.toFixed(2);

        // Scale percentage
        const scalePercent = (scaleX * 100)?.toFixed(2);

        // Position percentages relative to canvas
        const positionLeftPercent = ((posX / canvas.getWidth()) * 100)?.toFixed(2);
        const positionTopPercent = ((posY / canvas.getHeight()) * 100)?.toFixed(2);
                ...
                <div className={styles.toolbarBoxFontValueSetInnerActionheading} onClick={toggleTextColorPopup}>
                  ...
                  {/* <span><AngleActionIcon /></span> */}
                                    ...
                                    <div className={styles.inches}>
                                        <span>Height: {inches.heightIn} IN</span>
                                        <span>Width: {inches.widthIn} IN</span>
                                    </div>
