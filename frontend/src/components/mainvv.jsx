...
const MainDesignTool = ({
    ...
    const publishRenderedSize = (obj) => {
        if (!obj || !obj.id) return;
        const renderWidthCanvasPx = Math.round(obj.getScaledWidth());
        const renderHeightCanvasPx = Math.round(obj.getScaledHeight());

        dispatch(updateImageState({
            id: obj.id,
            changes: {
                renderWidthCanvasPx,
                renderHeightCanvasPx,
                canvasWidthPx: fabricCanvasRef.current?.getWidth() || 0,
                canvasHeightPx: fabricCanvasRef.current?.getHeight() || 0,
            }
        }));
    };
    ...
    useEffect(() => {
        ...
        const handleObjectAdded = (e) => {
            ...
            syncMirrorCanvasHelper(activeSide);
            const obj = e.target;
            if (obj && obj.type === 'image') publishRenderedSize(obj);
        };
